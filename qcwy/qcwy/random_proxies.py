...
class MongoCache_qcwy(object):
    ...
    def __init__(self):
        ...
        qcwy = self.db.qcwy
        self.db.qcwy.create_index('url')

    def __setitem__(self, key, value):
        record = {"url": str(value)}
        self.db.qcwy.update({'url': str(value)}, {'$set': record}, upsert=True)


    def __getitem__(self, item):
        record = self.db.qcwy.find_one({'url':str(item)})
        if record:
            return record['url']
        else:
